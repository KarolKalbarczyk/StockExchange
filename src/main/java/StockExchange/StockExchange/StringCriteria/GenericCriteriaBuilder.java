...
public abstract class GenericCriteriaBuilder implements CriteriaBuilder {
    ...
    public  <T> Criteria<T> valueInRange(Class<T> clazz, Attribute<T,?> attr, double min, double max){
        var query = String.format(Locale.ENGLISH," %s.%s between %s and %s",clazz.getSimpleName().toLowerCase(),attr.getName(),min,max);
        return buildCriteria(query,clazz);
    }

    public <T> Criteria<T> valueEqual(Class<T> clazz, Attribute<T,?> attr, double value){
        var query = String.format(Locale.ENGLISH," %s.%s = %s",clazz.getSimpleName().toLowerCase(),attr.getName(),value);
        return buildCriteria(query,clazz);
    }
    ...
    private <T,K> Criteria<T> joinBasedOnAttrName(String attrName,Class<T> clazz, List<Criteria<K>> criterias,Class<K> kClass){
        var criteria = new Criteria<T>();
    ...
    private <T> List<String> getFieldFromCriterias(
            List<Criteria<T>> criteria, Function<Criteria<T>
            ,List<String>> fieldSelect){
        return criteria.stream().map(fieldSelect)
